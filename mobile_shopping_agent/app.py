...
@app.route('/submit_device_data', methods=['POST'])
def submit_data():
    """
    Handles the form submission, extracts all fields from the POST request,
    and returns a summary of the received data.
    """
    if request.method == 'POST':
        # 1. Access the form data dictionary
        form_data = request.form

        # 2. Extract specific values by their 'name' attributes from the HTML form
        brand = form_data.get('brand')
        model = form_data.get('model')
        price = form_data.get('price')
        storage = form_data.get('storage')
        ram = form_data.get('ram')
        display_size = form_data.get('display_size')
        ...
        # 3. (Optional) Convert numerical strings to appropriate types
        try:
            price = float(price)
            ram = int(ram)
            rating = float(rating)
            # battery and display_size conversions can be added here too
        except (ValueError, TypeError):
            # Handle cases where conversion fails (e.g., if input wasn't strictly numeric)
            print("Warning: Could not convert some fields to numbers.")
        
        response = supabase.insert_data({
            "brand": brand,
            "model": model,
            "price": price,
            "storage": storage,
        ...
        # 5. Return a confirmation message to the user
        return f"Data received successfully for **{brand} {model}**! Check your console."    
